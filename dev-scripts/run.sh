...
sudo docker run -d --name react_cont --network=dnet --mount type=bind,source="$(pwd)"/../react-website/public/themes-dist,target=/usr/share/nginx/html/themes-dist --mount type=bind,source="$(pwd)"/../react-website/public/wet-boew-dist,target=/usr/share/nginx/html/wet-boew-dist react
